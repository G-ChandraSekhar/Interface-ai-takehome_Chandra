# MERIDIAN Adaptation — Development Log

> The deliverable write-up is [`ADAPTATION.md`](ADAPTATION.md), which is the
> 1-2 pages the brief asks for. This is the working record behind it: every
> decision, every defect, and the reasoning at the time. Kept because the
> reasoning is the part that is hard to reconstruct later, not because a
> reviewer should have to read it.
